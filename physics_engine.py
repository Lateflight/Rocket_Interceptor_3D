import grapher as grp
import numpy as np
import PiForce as PF
import Rotator_module as rt
...
Rk = rock.Rocket(m, Bottom-Center, Top-Center, thrust)   # holds I_principal, I_inv
...
tar = np.array([200, 10, 13000.0]) # m, target position (world)
vel_tar = np.array([100,50.0,0])   # m/s, target velocity (world)
...
# Simulation parameters
# ─────────────────────────────────────────
SIM_LEN     = 2500    # max iterations (25 s at dt = 0.01)
...
for i in range(SIM_LEN):
    ...
    if abs(alpha)>0 or Bottom_world[2]>0:
        mg = PF.g(m)
        ...
        Rk = rock.Rocket(m, Bottom-Center, Top-Center, thrust)
    ...
    gyro_accum = gyro_accum + w_meas * dt
